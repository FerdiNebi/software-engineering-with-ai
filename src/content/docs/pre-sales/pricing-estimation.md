...
- **Developer-task estimation** is the granular sizing of individual work items. It belongs further down the lifecycle — at [Discovery → Estimation & Cost Commitment](/discovery/estimation-cost-commitment/) once requirements are concrete, at [Discovery → Prototyping & Proof of Concept](/discovery/prototyping-proof-of-concept/) when a specific technical assumption needs validation, and at [Delivery → Estimation & Sprint Slicing](/delivery/project-management/estimation-sprint-slicing/) when sprint-level sizing happens against the backlog. Pre-sales pricing draws on developer-estimate intuition but is not the same artifact. The same word — "estimation" — sits at three points in the lifecycle, each with a different audience, fidelity, and commercial weight: pre-sales pricing under uncertainty → discovery re-estimate against scope → delivery-time sprint estimation against backlog items.
