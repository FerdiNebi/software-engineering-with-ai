...
Performance work runs continuously through development as a quality discipline, with named investment points: budget-setting at architecture time, baseline establishment when a feature first ships, regression detection in CI, and load testing in pre-launch handoff to [QA performance testing](/delivery/qa-testing/performance-testing/). The output is a system that meets the signed NFRs, with documented budgets and active regression detection that survives the engagement and operates post-handoff.
...
- Pre-launch load-test results showing the system holds under documented concurrent-user assumptions, with any findings triaged before [QA performance testing](/delivery/qa-testing/performance-testing/) starts
